...
func convert_symbols():
	...
	if animate_symbol.atlases.is_empty():
		...
		return
	var atlas = animate_symbol.atlases[animate_symbol.atlas_index]
	if not atlas:
...
func add_animation_from_symbol(node_path: NodePath, symbol: String, animate_symbol: AnimateSymbol):
	var frame_count
	var fps = animate_symbol.atlases[animate_symbol.atlas_index].get_framerate()
	if animate_symbol.atlases[animate_symbol.atlas_index] is AdobeAtlas:
		frame_count = animate_symbol.atlases[animate_symbol.atlas_index].get_length_of(symbol)
	else:
		frame_count = animate_symbol.atlases[animate_symbol.atlas_index].get_count_filtered(symbol)
